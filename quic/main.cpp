...
int main(){
   ...
   printf("========== Send initial packet ==========\n");
    ...
    quic::ParsePacket p;
    // server initial key
    std::vector<uint8_t> server_initial_hp_key = initial_secret_generator.server_hp_key();
    std::vector<uint8_t> server_initial_iv = initial_secret_generator.server_iv();
    std::vector<uint8_t> server_initial_key = initial_secret_generator.server_key();
    std::vector<uint8_t> header;
    std::vector<uint8_t> decoded_payload;

    //struct quic::PacketInfo packet_info = p.UnprotectHeader(packet, packet_size, server_initial_hp_key, EVP_aes_128_ecb(), header);
    struct quic::PacketInfo packet_info = p.Unprotect(
        packet, packet_size,
        server_initial_hp_key,
        server_initial_iv,
        server_initial_key,
        header,
        decoded_payload
    );
    id_of_server = packet_info.source_connection_id; // updated to choosed id by server
    ...
    // read crypto_frame
    quic::CryptoFrame *crypto_frame = reinterpret_cast<quic::CryptoFrame*>(initial_frame.get());
    ...
    std::copy(server_hello_bin.begin(), server_hello_bin.end(), std::back_inserter(hello_message));

    tls::Hash hash;
    size_t hash_length = 32;
    std::vector<uint8_t> hello_hash = hash.ComputeHash(hash_length, hello_message);

    tls::KeySchedule key_schedule;
    key_schedule.ComputeHandshakeKey(
    ...
    buf_pointer = 0;
    std::unique_ptr<quic::QUICFrame> handshake_frame = frame_parser.Parse(decoded_payload, buf_pointer);
    quic::CryptoFrame* crypto_frame_handshake = reinterpret_cast<quic::CryptoFrame*>(handshake_frame.get());
    ...
    // verify data
    {
        ...
        std::copy(
            server_hello_bin.begin(), server_hello_bin.end(), std::back_inserter(merged_handshake)
        );
        std::vector<uint8_t> handshake_server_hello = crypto_frame_handshake->GetPayloadWithoutFinished();
        std::copy(handshake_server_hello.begin(), handshake_server_hello.end(),
          std::back_inserter(merged_handshake)
        );

        std::vector<uint8_t> finished_hash = hash.ComputeHash(hash_length, merged_handshake);

        tls::HMAC hmac;
        std::vector<uint8_t> finished_key = key_schedule.GetServerFinishedKey();
        std::vector<uint8_t> verify_data = hmac.ComputeHMAC( finished_hash, finished_key);

        std::vector<uint8_t> server_sent_finished = crypto_frame_handshake->ServerSentFinished();
    ...
    std::vector<uint8_t> handshake_server_hello_to_server_fin = crypto_frame_handshake->GetPayload();
    std::copy(handshake_server_hello_to_server_fin.begin(), handshake_server_hello_to_server_fin.end(), 
        std::back_inserter(merged_handshake)
    );
    ...
    std::vector<uint8_t> client_handshake_hp  = key_schedule.GetClientHandshakeHP();
    std::vector<uint8_t> client_handshake_key = key_schedule.GetClientHandshakeKey();
    std::vector<uint8_t> client_handshake_iv  = key_schedule.GetClientHandshakeIV();
