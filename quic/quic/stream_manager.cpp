...
std::vector<uint8_t> Stream::GetPayload(){
    ...
    std::copy(stream_id_binary.begin(), stream_id_binary.end(), std::back_inserter(stream_payload));
    std::vector<uint8_t> length_binary = length.GetBinary();
    std::copy(length_binary.begin(), length_binary.end(), std::back_inserter(stream_payload));
    std::copy(payload_.begin(), payload_.end(), std::back_inserter(stream_payload));
