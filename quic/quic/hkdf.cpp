...
std::vector<uint8_t> HKDF::Extract(size_t hash_len, const std::vector<uint8_t> & salt, const std::vector<uint8_t> & ikm){
    std::unique_ptr<Botan::KDF> hkdf(Botan::KDF::create(std::string("HKDF-Extract(HMAC(SHA-256))")));
...
std::vector<uint8_t> HKDF::ExpandLabel(std::vector<uint8_t>  & secret, std::string label_string, std::vector<uint8_t> & context, size_t key_length){
    std::unique_ptr<Botan::KDF> hkdf(Botan::KDF::create(std::string("HKDF-Expand(HMAC(SHA-256))")));
    std::string label = {
        static_cast<char>((key_length&0xff00)>>8), 
...
std::vector<uint8_t> HKDF::DeriveSecret(size_t hash_length, std::vector<uint8_t> & secret, std::string label, std::vector<uint8_t> & message){

    tls::Hash hash;
    std::vector<uint8_t> transcript_hash = hash.ComputeHash(hash_length, message);
    std::vector<uint8_t> ret = ExpandLabel(secret, label, transcript_hash, hash_length);
    return ret;
}
} // namespace quic
