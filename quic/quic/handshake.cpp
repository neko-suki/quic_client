...
void Handshake::Protect(std::vector<uint8_t> & client_key, std::vector<uint8_t> & client_iv, std::vector<uint8_t> & client_hp_key){
...
void Handshake::CreateClientHandshake(std::vector<uint8_t> & scid, std::vector<uint8_t> & dcid, std::vector<uint8_t> & verify_data){
...
void Handshake::CreateCryptoFrame(std::vector<uint8_t> & verify_data){
    ...
    std::copy(finished_length, finished_length + 3, std::back_inserter(handshake));

    std::copy(verify_data.begin(), verify_data.end(), std::back_inserter(handshake));
    ...
    std::copy(offset_binary.begin(), offset_binary.end(), std::back_inserter(crypto_frame));
    VariableLengthInteger length(handshake.size());
    std::vector<uint8_t> length_binary = length.GetBinary();
    std::copy(length_binary.begin(), length_binary.end(), std::back_inserter(crypto_frame));
    std::copy(handshake.begin(), handshake.end(), std::back_inserter(crypto_frame));

    std::copy(
        crypto_frame.begin(), crypto_frame.end(),
...
void Handshake::CreateACKFrame(){
    ...
    std::copy(ack_frame_binary.begin(), ack_frame_binary.end(), std::back_inserter(payload_));
}

void Handshake::CreatePaddingFrame(){
    std::vector<uint8_t> padding_frame_binary = GeneratePaddingFrame(1100-payload_.size());
    std::copy(padding_frame_binary.begin(), padding_frame_binary.end(), std::back_inserter(payload_));
}

void Handshake::CreateHeader(std::vector<uint8_t> & scid, std::vector<uint8_t> & dcid){
