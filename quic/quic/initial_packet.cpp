...
void InitialPacket::CreateInitialPacket(std::vector<uint8_t> &scid,
  ...
  std::vector<uint8_t> padding_frame_binary =
      GeneratePaddingFrame(1162 - crypto_frame_binary.size());

  std::copy(crypto_frame_binary.begin(), crypto_frame_binary.end(),
...
tls::ECDH InitialPacket::GetECDH() { return crypto_frame_.GetECDH(); }

std::vector<uint8_t> InitialPacket::GetClientHello() {
  return crypto_frame_.GetClientHello();
}

void InitialPacket::CreateHeader(std::vector<uint8_t> &scid,
                                 std::vector<uint8_t> &dcid) {
  // first byte;
