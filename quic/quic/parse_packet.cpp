...
struct PacketInfo ParsePacket::Unprotect(
    ...
    struct PacketInfo packet_info = UnprotectHeader(packet, packet_sz, hp_key, cipher_suite, header, dcid);
...
struct PacketInfo ParsePacket::UnprotectHeader(unsigned char packet[], int packet_sz,
    const std::vector<uint8_t> & key, const EVP_CIPHER* cipher_suite, std::vector<uint8_t> & header, std::vector<uint8_t> dcid
){
    header.clear();
    int header_type;
    int pn_offset;
    ...
    if (EVP_CipherInit(evp, cipher_suite, key.data(), NULL, mode) != SSL_SUCCESS){
...
void ParsePacket::UnprotectPayload(
    std::vector<uint8_t> & header,
    ...
    if (EVP_CipherInit(evp, EVP_aes_128_gcm(), key.data(), nonce, mode) != SSL_SUCCESS){
        fprintf(stderr, "ERROR: EVP_EncryptInit\n");
        return;
    }

    int payloadl;
    if (EVP_CipherUpdate(evp, NULL, &payloadl, associated_data, associated_data_size) != SSL_SUCCESS){
    ...
    if (EVP_CIPHER_CTX_ctrl(evp, EVP_CTRL_AEAD_SET_TAG, tag_sz, tmp_tag) != SSL_SUCCESS){
        fprintf(stderr, "ERROR: EVP_CIPHER_CTX_ctrl(DEC)\n");
        return;
    }

    int test = EVP_CipherFinal_ex(evp, original_payload + tmp_len, &original_payload_sz);
