...
public:
    Handshake();
    void CreateClientHandshake(std::vector<uint8_t> & scid, std::vector<uint8_t> & dcid, std::vector<uint8_t> & verify_data);
    void Protect(std::vector<uint8_t> & client_key, std::vector<uint8_t> & client_iv, std::vector<uint8_t> & client_hp_key);
