...
void PacketProtection::Protect(
    ...
    ProtectPayload(
        ...
        protected_payload.data(), &protected_payload_sz,
        key.data(), key.size(),
        iv.data(), iv.size(),
        tag.data(),
        packet_number
    ...
    ProtectHeader(
        header.data(), header.size(),
        protected_payload.data(), protected_payload.size(),
        hp_key.data(),
...
int PacketProtection::ProtectPayload(
        ...
        const uint8_t key[], const int key_sz,
        const uint8_t iv[], const int iv_sz,
    ...
    if (EVP_CipherUpdate(evp, NULL, protected_payload_sz, associated_data, associated_data_sz) != SSL_SUCCESS){
        fprintf(stderr, "ERROR: EVP_CipherUpdate\n");
        EVP_CIPHER_CTX_free(evp);
        return 0;
    }

    if (EVP_CipherUpdate(evp, protected_payload, protected_payload_sz, payload, payload_sz) != SSL_SUCCESS){
        fprintf(stderr, "ERROR: EVP_CipherUpdate\n");
        EVP_CIPHER_CTX_free(evp);
        return 0;
    }

    if (EVP_CipherFinal(evp, protected_payload, protected_payload_sz) != SSL_SUCCESS){
