...
namespace tls {
void ClientHello::CreateClientHello(std::vector<uint8_t> &initial_source_connection_id) {
    std::vector<uint8_t> tmp;

    // legacy version
    legacy_version_ = 0x0303;
    tmp.push_back(
        static_cast<uint8_t>((legacy_version_ & 0xff00) >> 8));
    tmp.push_back(static_cast<uint8_t>((legacy_version_ & 0xff)));

    // copy and paste from RFC9001
    uint8_t random_input[32] = {0xeb, 0xf8, 0xfa, 0x56, 0xf1, 0x29, 0x39, 0xb9,
                                0x58, 0x4a, 0x38, 0x96, 0x47, 0x2e, 0xc4, 0x0b,
                                0xb8, 0x63, 0xcf, 0xd3, 0xe8, 0x68, 0x04, 0xfe,
                                0x3a, 0x47, 0xf0, 0x6a, 0x2b, 0x69, 0x48, 0x4c};
    ...
    std::copy(extension.begin(), extension.end(), std::back_inserter(extensions));
    ...
    std::copy(extension.begin(), extension.end(), std::back_inserter(extensions));
    quic_transport_parameter_.SetInitialSourceConnectionID(initial_source_connection_id);
    ...
    std::copy(tmp.begin(), tmp.end(),
              std::back_inserter(client_hello_));
}

std::vector<uint8_t> ClientHello::GetBinary(){
