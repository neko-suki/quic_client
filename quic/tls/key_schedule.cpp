...
void KeySchedule::ComputeHandshakeKey(size_t hash_length,
  ...
  // handshake secret
  handshake_secret_ = hkdf.Extract(hash_length, derived_secret_, shared_secret);
  ...
  client_handshake_key_ = hkdf.ExpandLabel(csecret_, label, empty_context, 16);

  // client handshake iv
  label = "quic iv";
  client_handshake_iv_ = hkdf.ExpandLabel(csecret_, label, empty_context, 12);

  // client handshake hp
  label = "quic hp";
  client_handshake_hp_ = hkdf.ExpandLabel(csecret_, label, empty_context, 16);
  ...
  label = "quic key";
  server_handshake_key_ = hkdf.ExpandLabel(ssecret_, label, empty_context, 16);

  // server handshake iv
  label = "quic iv";
  server_handshake_iv_ = hkdf.ExpandLabel(ssecret_, label, empty_context, 12);

  // server handshake hp
  label = "quic hp";
  server_handshake_hp_ = hkdf.ExpandLabel(ssecret_, label, empty_context, 16);

  // For verification?
  label = "finished";
  finished_key_ = hkdf.ExpandLabel(csecret_, label, empty_context, hash_length);
  server_finished_key_ =
      hkdf.ExpandLabel(ssecret_, label, empty_context, hash_length);
}

void KeySchedule::ComputeApplicationKey(size_t hash_length,
  ...
  std::vector<uint8_t> empty_message;
  std::string label = "derived";
  derived_secret_for_master_secret_ =
      hkdf.DeriveSecret(hash_length, handshake_secret_, label, empty_message);

  // master secret
  std::vector<uint8_t> zero_key(hash_length, 0);
  master_secret_ =
      hkdf.Extract(hash_length, derived_secret_for_master_secret_, zero_key);
  ...
  // client app key
  label = "quic key";
  client_app_key_ = hkdf.ExpandLabel(cmaster_secret_, label, empty_context, 16);

  // client app iv
  label = "quic iv";
  client_app_iv_ = hkdf.ExpandLabel(cmaster_secret_, label, empty_context, 12);

  // client app hp
  label = "quic hp";
  client_app_hp_ = hkdf.ExpandLabel(cmaster_secret_, label, empty_context, 16);
  ...
  // server app key
  label = "quic key";
  server_app_key_ = hkdf.ExpandLabel(smaster_secret_, label, empty_context, 16);

  // server app iv
  label = "quic iv";
  server_app_iv_ = hkdf.ExpandLabel(smaster_secret_, label, empty_context, 12);

  // server app hp
  label = "quic hp";
  server_app_hp_ = hkdf.ExpandLabel(smaster_secret_, label, empty_context, 16);
}
...
std::vector<uint8_t> KeySchedule::GetFinishedKey() { return finished_key_; }

std::vector<uint8_t> KeySchedule::GetServerFinishedKey() {
  return server_finished_key_;
}

std::vector<uint8_t> KeySchedule::GetServerAppKey() { return server_app_key_; }
std::vector<uint8_t> KeySchedule::GetServerAppHP() { return server_app_hp_; }
std::vector<uint8_t> KeySchedule::GetServerAppIV() { return server_app_iv_; }

std::vector<uint8_t> KeySchedule::GetClientAppKey() { return client_app_key_; }
std::vector<uint8_t> KeySchedule::GetClientAppHP() { return client_app_hp_; }
std::vector<uint8_t> KeySchedule::GetClientAppIV() { return client_app_iv_; }
...
std::vector<uint8_t> KeySchedule::ComputeHash(size_t hash_length,
