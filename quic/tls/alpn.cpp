...
std::vector<uint8_t> ALPN::GetBinary() {
    ...
    for (const auto &protocol_name : protocol_name_list) {
      uint8_t protocol_name_length[1] = {static_cast<uint8_t>(protocol_name.size() & 0xff)};
    ...
    uint8_t buf_length[2] = {static_cast<uint8_t>((buf.size() & 0xff00) >> 8), static_cast<uint8_t>(buf.size() & 0xff)};
    ...
    uint8_t alpn_extentino_length[2] = {static_cast<uint8_t>((extension.size() & 0xff00) >> 8),
