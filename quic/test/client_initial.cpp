#include <vector>
#include <cassert>

#include "../quic/packet_protection.hpp"

void TestRFC9001A2(){
    std::vector<uint8_t> header = {
        0xc3,0x00,0x00,0x00,0x01,0x08,0x83,0x94,0xc8,0xf0,0x3e,0x51,0x57,0x08,0x00,0x00,0x44,0x9e,0x00,0x00,0x00,0x02,
    };
    std::vector<uint8_t> payload = {
        ,
    };
    std::vector<uint8_t> padding(1162-payload.size(), 0);
    std::copy(padding.begin(), padding.end(), std::back_inserter(payload));
    std::vector<uint8_t> key = {0x1f,0x36,0x96,0x13,0xdd,0x76,0xd5,0x46,0x77,0x30,0xef,0xcb,0xe3,0xb1,0xa2,0x2d,};
    std::vector<uint8_t> iv = {0xfa,0x04,0x4b,0x2f,0x42,0xa3,0xfd,0x3b,0x46,0xfb,0x25,0x5c,};
    std::vector<uint8_t> hp_key = {
        0x9f,0x50,0x44,0x9e,0x04,0xa0,0xe8,0x10,0x28,0x3a,0x1e,0x99,0x33,0xad,0xed,0xd2,
    };
    uint64_t packet_number = 2;
    int packet_number_offset = 18;
    std::vector<uint8_t> protected_payload(payload.size());
    std::vector<uint8_t> tag(AES_BLOCK_SIZE);

    quic::PacketProtection packet_protection;
    packet_protection.Protect(header, payload, key, iv, hp_key, packet_number, packet_number_offset, protected_payload, tag);

    std::vector<uint8_t> quic_packet;
    std::copy(header.begin(), header.end(), std::back_inserter(quic_packet));
    std::copy(protected_payload.begin(), protected_payload.end(), std::back_inserter(quic_packet));
    std::copy(tag.begin(), tag.end(), std::back_inserter(quic_packet));

    std::vector<uint8_t> expected_quic_packet = {
        ,
    };

    assert(quic_packet == expected_quic_packet);
}

void TestQUICXargsOrg(){
    std::vector<uint8_t> header = {
        0xc0,0x00,0x00,0x00,0x01,
        0x08,0x00,0x01,0x02,0x03,
        0x04,0x05,0x06,0x07,0x05,
        0x63,0x5f,0x63,0x69,0x64,
        0x00,0x41,0x03,0x00,
    };
    std::vector<uint8_t> payload = {
        
    };
    
    std::vector<uint8_t> key = {0xb1,0x4b,0x91,0x81,0x24,0xfd,0xa5,0xc8,0xd7,0x98,0x47,0x60,0x2f,0xa3,0x52,0x0b,};
    std::vector<uint8_t> iv = {0xdd,0xbc,0x15,0xde,0xa8,0x09,0x25,0xa5,0x56,0x86,0xa7,0xdf,};
    std::vector<uint8_t> hp_key = {
        0x6d,0xf4,0xe9,0xd7,0x37,0xcd,0xf7,0x14,0x71,0x1d,0x7c,0x61,0x7e,0xe8,0x29,0x81,
    };
    uint64_t packet_number = 0;
    int packet_number_offset = 23;
    std::vector<uint8_t> protected_payload(payload.size());
    std::vector<uint8_t> tag(AES_BLOCK_SIZE);

    quic::PacketProtection packet_protection;
    packet_protection.Protect(header, payload, key, iv, hp_key, packet_number, packet_number_offset, protected_payload, tag);

    std::vector<uint8_t> quic_packet;
    std::copy(header.begin(), header.end(), std::back_inserter(quic_packet));
    std::copy(protected_payload.begin(), protected_payload.end(), std::back_inserter(quic_packet));
    std::copy(tag.begin(), tag.end(), std::back_inserter(quic_packet));

    std::vector<uint8_t> expected_quic_packet = {
        ,
    };

    assert(quic_packet == expected_quic_packet);
}

int main(){
    TestRFC9001A2();
    TestQUICXargsOrg();
    return 0;
}
